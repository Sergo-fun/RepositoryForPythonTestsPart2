...
def test_server_unavailable(api_client, mocker):
    ...
    with pytest.raises(AssertionError, match="Expected status code of 201, got 405"):
        api_client.ping()
...
def test_server_unavailable(api_client, mocker):
    ...
    mocker.patch.object(api_client.session, 'get', return_value=mock_response)
    with pytest.raises(AssertionError, match="Expected status code of 201, got 500"):
        api_client.ping()
...
def test_server_unavailable(api_client, mocker):
    ...
    mocker.patch.object(api_client.session, 'get', return_value=mock_response)
    with pytest.raises(AssertionError, match="Expected status code of 201, got 404"):
        api_client.ping()
...
def test_server_unavailable(api_client, mocker):
    ...
    mocker.patch.object(api_client.session, 'get', return_value=mock_response)
    with pytest.raises(AssertionError, match="Expected status code of 201, got 200"):
        api_client.ping()

@allure.feature('Test Ping')
@allure.story('Test timeout')
def test_ping_timeouts(api_client, mocker):
